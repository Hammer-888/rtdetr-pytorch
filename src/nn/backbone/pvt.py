...
class PVT(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            if download_url[self.variant]:
                state = torch.hub.load_state_dict_from_url(download_url[self.variant])
                self.load_state_dict(state, strict=False)
                print(f"Load PVT-{self.variant} state_dict")
            else:
                print(f"Warning: No download URL provided for {self.variant}")
